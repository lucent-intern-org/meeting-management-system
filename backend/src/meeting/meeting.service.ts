...
export class MeetingService {
    ...
    async getDate(date :string){
        let meeting = await this.meetingRepository.find({where:{date},
        });
        return meeting;
    }
    async getMeeting(meeting){
        return await this.meetingRepository.findOne({where:{roomId: meeting.roomId,date: meeting.date,
            startTime:meeting.startTime,endTime:meeting.endTime,
            title:meeting.title,content:meeting.content,repeat:meeting.repeat},
        });
    }
    async getAll(){
        let meeting = await this.meetingRepository.find();
        return meeting;
    }
    async createMeeting(date,meeting){
        return await this.meetingRepository.save({roomId: meeting.roomId,date: date,
            startTime:meeting.startTime,endTime:meeting.endTime,
            title:meeting.title,content:meeting.content,repeat:meeting.repeat});
